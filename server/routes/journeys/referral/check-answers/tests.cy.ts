...
context('test /record-investigation/check-answers', () => {
  ...
  it('should be able to change answers and proceed to confirmation', () => {
    ...
    cy.findByRole('textbox').should('be.focused').clear().type('Not Jimmy', { delay: 0 })
    getContinueButton().click()
    cy.url().should('to.match', /check-answers/)
    cy.contains('dt', 'Names of staff assaulted').next().should('include.text', `Not Jimmy`)
