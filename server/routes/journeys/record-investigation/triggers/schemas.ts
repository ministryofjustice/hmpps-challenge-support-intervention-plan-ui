...
const UPDATE_ERROR_MSG = 'Enter an update to the description of the prisoner’s triggers'
...
export const schemaFactory = async (req: Request, res: Response) => {
  ...
  return createSchema({
    personsTrigger: z
      ...
      .max(
        maxLengthChars,
        `Description of the prisoner’s triggers must be ${maxLengthChars.toLocaleString()} characters or less`,
      )
