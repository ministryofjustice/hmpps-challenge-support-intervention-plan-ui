...
context('test /record-investigation/check-answers', () => {
  ...
  it('should be able to change answers and proceed to confirmation', () => {
    ...
    cy.contains('dt', 'Risks and triggers')
      .next()
      .should('include.text', `<script>alert('xss-personsTrigger');</script>`)
    cy.findByRole('link', { name: /change the description of the prisoner’s risks and triggers/i })
      .should('be.visible')
      .click()
    cy.url().should('to.match', /triggers(#[A-z]+)?$/)
    cy.findByRole('textbox').clear().type('new text', { delay: 0 })
    cy.findByRole('button', { name: /Continue/i }).click()
    cy.contains('dt', 'Risks and triggers').next().should('include.text', 'new text')
