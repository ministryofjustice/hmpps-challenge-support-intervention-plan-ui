...
context('test /record-investigation/usual-behaviour-presentation', () => {
  ...
  const getInputTextbox = () =>
    cy.findByRole('textbox', { name: 'What’s Testname User’s usual behaviour presentation?' })
  ...
  const validatePageContents = () => {
    cy.findByRole('heading', { name: 'What’s Testname User’s usual behaviour presentation?' }).should('be.visible')
    cy.findByText(/Where to find this information/).should('be.visible')
